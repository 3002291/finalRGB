...
import com.douzone.comet.service.eh.ehsevr.dao.Ehsevr00200_DAO;
...
/** 
  ...
  * @Author  : 최진웅
  */
@DzApiService(value="Ehsevr00200_Service", module=CometModule.EH, desc="배출시설등록", version ="1.0.22110201")
public class Ehsevr00200_Service extends DzCometService {

	@Autowired
	Ehsevr00200_DAO Ehsevr00200_dao;
	
	@Autowired
	DzMybatisSupport mybatisSupport;

	@DzApi(url="/Ehsevr00200_list", desc="배출시설등록-조회", httpMethod=DzRequestMethod.GET)
	public List<Ehsevr00200> Ehsevr00200_list(
			@DzParam(key="plant_cd", desc="사업장", paramType=DzParamType.QueryString) String plant_cd,
			@DzParam(key="dschfclt_nm", desc="배출시설명", paramType=DzParamType.QueryString) String dschfclt_nm
	) throws Exception {
		List<Ehsevr00200> list =  new ArrayList<Ehsevr00200>();
		try {
			HashMap<String, Object> parameters = new HashMap<String, Object>();
			
			parameters.put("P_COMPANY_CD", this.getCompanyCode());
			parameters.put("P_PLANT_CD", plant_cd);
			parameters.put("P_DSCHFCLT_NM", dschfclt_nm);
			
			list = Ehsevr00200_dao.list("selectList", parameters);
	 	} catch(Exception e) {
	...
	@DzApi(url = "/ehsevr00200_save", desc = "배출시설등록 저장", httpMethod = DzRequestMethod.POST)
	public SCMApiProvider_Ps_ResultData ehsevr00200_save(
			@DzParam(key = "mstGird_ds", desc = "배출시설등록", paramType = DzParamType.Body) DzGridModel<Ehsevr00200> mstGird_ds
			) throws Exception {
		...
		try {
			...
			if (mstGird_ds != null) {
				if (mstGird_ds.getDeleted().size() > 0) {
					Ehsevr00200_dao.deleteEhsevr00200(mstGird_ds.getDeleted());
				}
				if (mstGird_ds.getUpdated().size() > 0) {
					Ehsevr00200_dao.updateEhsevr00200(mstGird_ds.getUpdated());
				}
				if (mstGird_ds.getAdded().size() > 0) {
					for (Ehsevr00200 item : mstGird_ds.getAdded()) {
						cnt = mybatisSupport.selectOne(MyBatisUtil.getId(this.getClass(), "dao.Ehsevr00200_DAO.mstDupCheck"),item);
						if(cnt > 0){
					...
					Ehsevr00200_dao.insertEhsevr00200(mstGird_ds.getAdded());
				}
			}
			transaction.commit();
		} catch (Exception e) {
